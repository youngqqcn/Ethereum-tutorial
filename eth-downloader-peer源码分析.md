...
```go
// peerConnection表示一个活动对等方，从该对等方检索哈希和块。
// peerConnection represents an active peer from which hashes and blocks are retrieved.
type peerConnection struct {
	id string // Unique identifier of the peer

    // 几种状态: 区块头, 区块体, 收据, 节点状态 
...
FetchHeaders FetchBodies, ....等函数 主要调用了eth.peer的功能来进行发送数据请求。

```go

// FetchHeaders将标头检索请求发送到远程对等方。
...
// FetchNodeData将节点状态数据检索请求发送到远程对等方。 
...
func (ps *peerSet) idlePeers(minProtocol, maxProtocol uint, idleCheck func(*peerConnection) bool, throughput func(*peerConnection) float64) ([]*peerConnection, int) {
    ...
    //  peerThroughputSort实现了Sort接口, 以便使用快速排序
	// And sort them
